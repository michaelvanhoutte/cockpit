...
**Panels** behave like the tiles on Azumuta's Analytics dashboards: each sits on a grid, **drag to move**, **drag a corner to resize**, an editable **title**, and a **menu of its own** (configure, rename, remove) opened by the same control as every other menu in the app (§11, "One control opens every menu"). **Adding a Panel happens at the foot of the Dashboard**, on a hairline strip under the last row of Panels, so a Panel is made where Panels live rather than from a toolbar that could not say where the new one would go. The responsive grid reflows on smaller screens so the same Dashboard works on phone and desktop.
...
**Changing the arrangement changes the Layout you are on, and asks nothing.** Dragging, moving from a Panel's own menu and resizing all go into it, whatever screen you are on. There used to be a question here — *change this Layout*, or *make a Layout for this screen* — and it existed because the Layout had been picked *for* you and either answer was reasonable. Picking it by name is what makes the gesture mean what it says.

**A Dashboard that has never been arranged is drawn to fit the screen it is on**: the Panels in the order they were added, filling the rows left to right, each an equal share of the width, at a count that keeps a Panel worth reading — one across on a phone, three on a laptop, four on anything wider. It has no Layout at all, and the first move or resize records one, named for the size of screen it was made on — *Phone*, *Tablet*, *Laptop* or *Wide*, and yours to change. **A Dashboard that has a Layout keeps one**, so deleting the last is refused with that sentence on the entry: what it protects is an arrangement somebody made, not the existence of a row.
