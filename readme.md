...
The first screen is the logon page, and the way in is *Continue with Google*. Locally that goes to a **stub issuer this stack runs** rather than to Google — it prints its address beside the other two — where you pick one of the two people the seed creates and come back signed in. It is the real code flow either way, and no deployed environment has a stub: see "App login" in [docs/architecture.md](docs/architecture.md).
...
- The browser tier starts a **second copy of the whole application** — its own Wrangler, its own Vite, its own state directory, on two more ports of this checkout's own (:8887 and :5273 in the primary checkout, derived in a worktree) — and throws the storage away afterwards. Your `pnpm dev` keeps running untouched.

That storage is rebuilt before every run, so a run can never be made to pass or fail by something you did in the browser yesterday. The **register** is rebuilt by copying a template (about 220KB, 5ms) rather than by running migrations and the seed (about seven seconds, nearly all process startup); the template is rebuilt only when a migration or `seed.sql` changes, keyed by their contents, so there is no stale-template failure to remember. **Each account's own store** is not in that template — a Durable Object cannot be written from outside — so each run starts them empty and the first request that opens one creates it and gives it the workspace, dashboard and panel an account starts with. A whole run costs about 7 seconds warm, 18 the first time.
...
|---|---|---|
| [CI](.github/workflows/ci.yml) | push to `main`, and pull requests to `main` | Six jobs, so a failure still names itself: `Checks` runs first — checkout, the classifier, then Scripts, Concepts, Typecheck, Lint and Build as its own steps rather than six separate jobs each billing a runner start-up ("Run the six small CI jobs as one, so a push stops paying six runner start-ups", issue 378) — `Test` and `E2E (F3)` wait on it, `Stability` runs beside it, `Test Explorer` waits on `Test` and `Checks`, and `Publish` waits on `Test Explorer` and `Stability`. `Checks`, `Test` and `E2E (F3)` are three of the five contexts the branch-protection payload lists (the other two are the Claude reviews); `Checks`'s Concepts step checks the test-explorer registry, `Test Explorer` and `Stability` publish reports, and neither gates — `Publish` puts what the latter two produced on [GitHub Pages](https://michaelvanhoutte.github.io/cockpit/) from `main` only. `Checks`'s classifier step gates nothing of its own — it classifies the diff (`scripts/lib/what-changed.mjs`, taken from the base commit so a branch cannot rule on its own), and `Checks`'s own Typecheck, Lint, lint-config and Build/bundle-budget steps skip on a diff confined to `docs/`, `.claude/` and root-level Markdown, the same way `Test` and `E2E (F3)` do reading its output, with `Checks` itself still reporting green ("Skip the mechanical checks on a pull request that touches nothing they cover", issue 345); `Test Explorer` then skips too, since it waits on `Test` and a skipped dependency is not a successful one. `Lint` runs [`eslint.config.mjs`](eslint.config.mjs) over the repository and then the tests that hold its `files` globs — the three rules being the React Hooks pair, no focused test reaching `main`, and the L1/F1 dependency table as restricted imports. `E2E (F3)` installs Chromium and runs the browser tier against the same isolated local stack you get locally, keeping failure traces as an artifact. `Scripts` runs the unit tests for `scripts/lib/`, because a silent change in the test stack's guards would let a run start against a database it did not create, and one in the review gate would let a non-review ship green. `pull_request` — which tests the merge result, what actually lands — is the only event that checks an open branch, so a branch with no open pull request is checked by nothing; [`/issue`](.claude/commands/issue.md)'s own process already runs typecheck, lint, the tests and — where CLAUDE.md's Tests table calls for one — a browser pass by hand throughout a unit of work, and opens the pull request only once that is already done. |
| [CodeQL](.github/workflows/codeql.yml) | push to `main`, and a weekly schedule — never a pull request | Two legs: `javascript-typescript` over the application sources, and `actions` over the workflow files — where this repository's own risk sits, since its workflows run Claude against an OAuth token and check out pull request branches. Both read the sources directly (`build-mode: none`). None of its three contexts (the two legs, plus `CodeQL` itself, posted by GitHub Advanced Security when an analysis uploads results) is a required status check any more: "Decide whether CodeQL earns its run on every pull request push, or moves to main and a schedule" (issue 379) moved it off the pull-request gate on cost — 40 runs sampled found no CodeQL alert had ever held a pull request, against roughly 4 of a push's 39 billed minutes. An alert now surfaces within a week of merging rather than before (deployment.md, "Bootstrap runbook"). |
| [Claude Code Review](.github/workflows/claude-code-review.yml) | pull request opened, pushed to, reopened, ready for review — but never while it is a draft | Runs the `code-review` plugin command and posts findings as inline comments. A second step asserts the review *actually ran* — see below. A draft's pushes cost CI alone, and marking it ready fires the review once against the finished head; superseded runs are cancelled, so pushing twice in a minute leaves one review, not two. Also skips a documentation-only pull request, the same way it skips a fork, a draft or a bot pull request — its own classifier job reads the base commit's copy, the way `Checks`'s does ("Skip the code review on a documentation-only pull request, as CI already skips its jobs", issue 371). |
| [Claude Security Review](.github/workflows/claude-security-review.yml) | same | A security pass over the diff, scoped by [.github/security-review-instructions.md](.github/security-review-instructions.md) to this project's own rules — the only per-pull-request coverage for injection, taint and workflow misuse now that CodeQL only analyses `main`. The run must end with a one-line verdict naming the highest severity found; the check goes red when that line is missing or names the top severity. Skipped on pull requests from forks — see below. |
...
**Why both gates are one module.** They answer the same question — did this run reach a verdict — and the code review answered it in about 140 lines of bash inside a `run:` block until "Give the code review the tested gate the security review already uses" (issue 277) moved it. Every incident recorded in that file's comments is a bug in that bash rather than in the reviewing: a `permission_denials_count` field read as "no denials", two blocked sessions passing as clean, a result payload that is sometimes an array and sometimes an object. All four shipped green, because inline bash cannot be run by a test. Both now decide in [`scripts/lib/review-gate.mjs`](scripts/lib/review-gate.mjs), a pure function from an execution record to a decision, asserted by `node --test`; the workflows call it through `scripts/assert-code-review.mjs` and `scripts/assert-security-review.mjs`, which do the file reading, the printing and the exit code and nothing else. Two functions rather than one, because the two ask different things of the same record: the security review states a verdict in a line it is required to write, and the code review's only evidence of one is that it posted. The move deliberately changed nothing either check decides; the rule change came next, and landed there — see "Why the gate asks about the head, and not about the pull request" below.
...
**Why the review has a gate step.** `claude-code-action` reports success when the *session* ended cleanly, which is not the same as a review having happened: earlier runs here were blocked by a denied tool, or ended waiting for a completion notification a one-shot run never sends, and every one was a green tick. The `Assert the review actually ran` step turns the check red unless Claude reached a verdict, the signal being that it *posted* something about the head being reviewed. That is not free: the plugin command may stop without a word on a change it judges too simple to review, so the workflow's system prompt tells it to post that verdict like any other — otherwise a review that reached the right answer is indistinguishable from one that never ran, which is how a correct review of "Delete the stray file a shell redirect left at the repository root" (pull request 80) came to fail this gate. Denied tool calls are fatal only when no verdict landed; the code review's turn count is never more than a warning, because a legitimately short run and a blocked one look alike.

**Why the security review no longer warns on turn count.** Seven of eight pull requests sampled carried the warning under a clean verdict, most 4 to 9 turns, and the count tracked nothing about the diff: a 448-line pull request took 4 turns and a 556-line one took 9 (the sampled numbers are in `decideSecurityOutcome`'s own doc-comment, `scripts/lib/review-gate.mjs`). A warning nearly every review carries is one nobody reads ("Make the security review warning mean something, or drop it", issue 284), so it is gone — the verdict already tells a reader whether the review found anything, which is what the turn count was standing in for. The denial warning stayed and got more useful: it now quotes the denial's own message rather than repeating the tool name, since every denial sampled named `Bash` and the message was the only part that said what was actually refused — a subagent shelling out to `grep | head` where the Grep tool already did the job, or, three denials in one pull request, a review trying to save the diff to a file, which Claude Code's sandbox blocks outright regardless of the allowlist.

**Why the gate asks about the head, and not about the pull request.** Having posted *at all* answers for the first round and for no round after it — round one's comments satisfy that test permanently, so from round two on the check could not tell "reviewed, found nothing" from "declined without looking". That is "The review check goes green when the reviewer declined to look at the new commits" (issue 75), and it went green on the fourth head of "Make managing the account a window over the workspace, not a screen instead of it" (pull request 193) before anyone opened the log. So the gate counts only what the reviewer said about the commit *this run* was handed: an inline finding by the commit it was made on — never the one GitHub later moved it to — and a summary comment by its timestamp, that being all an issue comment carries. The head is placed in time by the earliest workflow run GitHub created for it, never by the commit’s own committer date — which is written by whatever clock made the commit and bounds nothing, as `placeHead` sets out. Where no run date can be read, the check falls back to the whole pull request and says in a warning that it proved less. The reviewer is told the same rule in the appended system prompt, since a decline the gate merely refuses would go red every second round instead of being reviewed.
...
| `pnpm typecheck`, `pnpm lint`, `pnpm test`, `pnpm build` | The same four gates CI runs, so a red pipeline is reproducible locally. `pnpm lint` is the small rule set in [`eslint.config.mjs`](eslint.config.mjs); `pnpm test:fast` runs its tests along with the fast tiers. |
...
- **Branch protection on `main`**, whose payload *is* checked in — [.github/branch-protection.json](.github/branch-protection.json) — but which the owner still has to apply, because configuration nobody can review or restore is not really configuration:
  ...
  Because applying it is a separate act, **the payload and the live setting drift** and nothing fails when they disagree — the file listed eight required checks from 2026-08-31 while `main` enforced four, and it took until 2026-09-09 to be applied. Read the live one whenever the answer matters: `gh api repos/michaelvanhoutte/cockpit/branches/main/protection --jq '.required_status_checks.contexts'`.
