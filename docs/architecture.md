...
- **The operator's routes are behind a secret, not behind a role.** `/v1/admin/` hands an operator every account's data for a backup ("Take a backup of an environment, or of one user", issue 208), and it is gated by `BACKUP_TOKEN` rather than by the `admin` role every user already carries — because on a deployed environment signing in is choosing a name off a list, so a role gate would put all of it one click from anyone who reaches the URL. That is not the identity model these routes eventually want; it is the one that is stronger than what exists today, and it stops the first admin surface from waiting on "App login: hand-rolled Google OIDC + own sessions". An environment with no secret set refuses them outright rather than opening them. **The first admin-only *page* still brings the role check with it** — this is a command-line surface with no session, and the two do not substitute.
