...
const routes = app
  ...
  .openapi(commandRoute('set_item_type'), async (c) => c.json(await change(c, 'set_item_type', c.req.valid('json')), 200))
  .openapi(commandRoute('set_due_date'), async (c) => c.json(await change(c, 'set_due_date', c.req.valid('json')), 200))
