...
4. **More than one user, SaaS-ready tomorrow.** Per the functional definition's "Decisions already made": never hard-code how many accounts there are. Every row is tenant-scoped, and since "Sign in by picking a name, each user in their own account" (issue 86) that scoping is exercised rather than assumed - each user signs in, and the account every request resolves is the account of whoever signed in. Auth is real OAuth *eventually*, and today is not that day - see "App login: hand-rolled Google OIDC + own sessions" (§8.1) for what has shipped of it and what has not; secrets are stored encrypted, and workspace scoping is enforced server-side.
...
- **This is a recorded reversal.** Earlier drafts chose Postgres from day one, arguing that SaaS-ready demanded it and that "SQLite would suffice for one user and then charge interest on the migration forever." The Cloudflare hosting decision (§9) put that argument in conflict with the platform (Cloudflare has no managed Postgres), and the review compared three exits: add a second vendor for serverless Postgres (rejected: the most moving parts, cross-network latency on every query, and the data held hostage to another company's free-tier strategy), leave Cloudflare for a co-located-Postgres platform, or drop Postgres. Re-examined, the Postgres argument turned out to be doing *speculative* work while the Cloudflare arguments do *concrete* work today, so Postgres yielded:
  ...
  - **"SaaS-ready" is schema discipline, not an engine.** The functional definition's rule is "never hard-code single-tenancy into the schema"; `tenant_id` columns work identically in SQLite. And the credible multi-tenant shape on this platform is **one store per tenant**, which gives stronger isolation than row-scoping in a shared Postgres would. *Which* store was settled later, when multiple users forced the question, and the answer was not a second D1 database: see [account-storage-options.md](account-storage-options.md), which measured the alternatives and chose a Durable Object per account, with the account register staying in D1. See "One store per account, and `tenant_id` stays" below for what that does and does not mean.
...
- **`tenant_id` on every row**, non-null. It was written when there was one tenant and nothing exercised it; there is now more than one account, each in a store of its own, and the column stays because it is what makes a request that reached the wrong store match no row instead of somebody else's ("One store per account, and `tenant_id` stays", below). Workspace scoping (the privacy boundary of the functional definition's "Container hierarchy", §4.1) is enforced in queries server-side, never only in the UI.
...
- **A CHECK for every closed set**, built in `src/accounts/schema.ts` and `src/db/schema.ts` from the same Zod enums the wire contract uses, so the database and the API contract cannot drift into disagreeing about what a status is.
...
**The store is a Durable Object, not a second D1 database**, and that was measured rather than argued: [account-storage-options.md](account-storage-options.md) has the numbers, the conditions they were taken under, and what would reverse the decision. The short version is that a D1 database per account cannot be provisioned without a deploy — bindings are resolved when a Worker version is deployed — while a Durable Object is reached by name at runtime and created on first touch, and the price of that (each account applies outstanding schema changes inside somebody's request) turned out to be milliseconds rather than the unknown it was assumed to be.

Three things follow, and each shows up in the code:

- **Bringing an account up to date happens inside a request**, so when it fails the failure has to be legible: `apps/api/src/accounts/up-to-date.ts` names the change and keeps the underlying cause, because the off-the-shelf migrator reports only `Rollback` and loses the real error in the response and the logs alike.
- **The live-updates stream stays in the Worker**, polling the account's store rather than moving into it. Durable Objects bill wall-clock duration and an open connection keeps one in memory; the stream is long-lived by design.
- **The register cannot be joined to account data**, which the platform enforces: D1 cannot join across bindings, and a Worker cannot join D1 to a Durable Object at all. So `tenant_id` carries no foreign key inside a store — the register it would point at is in another database entirely.

**`tenant_id` stays on every row even after the split.** It looks redundant once a store holds one account, and it is not:

- **It is the second lock again.** Correctness otherwise rests entirely on addressing the right store. `tenant_id`, and the filters every query in `apps/api/src/accounts/repo.ts` already applies, is the check behind it. With it, a routing bug returns nothing; without it, that same bug silently serves another account's data — the worst available failure mode for a privacy boundary.
- **It is the row's provenance.** The moment a row leaves its store — a backup, an export, a support copy, a restore into the wrong place — `tenant_id` is the only thing that says whose it is.
...
- **Which layout a dashboard is drawn with is decided client-side too**, from the same snapshot: it carries every panel and every layout of the workspace, so switching dashboard, resizing the window and picking a layout by hand all reflow without a request (functional definition, "Layouts: one arrangement per screen size"). Only *changing* an arrangement is a write, and it is one command carrying the whole arrangement rather than one per gesture.
- **Liveness via SSE.** Phone and desktop are commonly open simultaneously, so the API pushes invalidation events (item changed, sync completed) over server-sent events; the client also revalidates on focus/visibility change. SSE over WebSockets because the channel is strictly server-to-client notifications and SSE is plain HTTP: simpler to run, to proxy, and to test. On Workers (§9) an idle SSE stream costs essentially nothing (CPU-time billing); if connection churn ever becomes a real problem, a Durable Object is the designated upgrade path, not a reason to complicate v1.

  **`EventSource` reconnects natively — but only from some failures, and not the ones that matter.** Measured 2026-08-31 over twenty seconds against the built app: a *dropped* connection is retried about every three seconds indefinitely, while a connection *answered* badly (a redirect to the sign-in page, or a `503`) is abandoned after a single attempt, permanently and silently. So the browser handles the failure that would heal anyway and gives up on the two that need handling. `apps/web/src/api/useServerEvents.ts` therefore replaces a permanently-closed stream itself, backing off 3s→60s, and asks the ungated `/health` first so an expired sign-in is surfaced through the same screen a failed read uses rather than announced twice.
...
- **App login per "App login: hand-rolled Google OIDC + own sessions" (§8.1)**; no passwords stored, ever. The session, cookie and request gate described there are built; the proof of identity in front of them is a logon page you sign in at by choosing a name, which is an identity selector rather than an authentication control, and Cloudflare Access is what actually authenticates a deployed environment until that changes.
...
- **Workspace scoping enforced server-side** on every query via `tenant_id` + workspace filters; the UI's scoping is presentation, not protection. The account those filters carry is the account of whoever signed in, resolved from the session on every request and never from anything the client sends.
...
- **No pull request merges unanalysed.** CodeQL reads the sources and the workflows on every pull request (the CI/CD section, §9.1); Dependabot reports dependencies with known vulnerabilities and opens the fix; secret scanning with push protection refuses a committed credential at the push rather than reporting it afterwards. These are the mechanical half, and they do not model this document's own rules — a webhook missing the ingress hardening template, a source token reaching a log, a workspace-scoped read with no tenant filter. Reviewing a change against *those* is the second, judgement half: a Claude pass over every pull request's diff, scoped by a checked-in instructions file rather than by generic advice, and required to end with an explicit verdict so that a run which never reached one is red rather than silent.
...
**Staged, and what has shipped so far (2026-09-01).** Everything above is built *except the proof of identity*. "Sign in by picking a name, each user in their own account" (issue 86) shipped the whole downstream half: a session row in D1, an httpOnly cookie with sliding expiry, a gate in front of every request that refuses in the application's own JSON rather than with a web page, users and accounts in the register, and an account resolved per request from the signed-in user. What stands in for the OIDC code flow is a **logon page listing the users, where you sign in by clicking a name**.

Said plainly, so that nobody later mistakes it for what it is not: **a passwordless list of names is an identity selector, not an authentication control.** It establishes *which* user you are for the purposes of scoping data; it establishes nothing at all about whether you are entitled to be that user. Two consequences follow and neither is optional:

- **Cloudflare Access stays in front of every deployed environment, unreconsidered.** The trigger for reconsidering it is password or OAuth login shipping - not this. Until then Access is the only thing actually authenticating anybody, and the note below stands unchanged.
- **When Google sign-in arrives it replaces one step**, how we come to believe who you are, and everything downstream of it is already built and tested. That was the whole point of building the session for real rather than faking it: the swap is the logon page and the callback, not the session model, the cookie, the gate, or the account boundary.

Two things ship as data with no behaviour behind them, deliberately. A **role** on each user (`user` or `admin`), so role logic is in the schema from the start rather than retrofitted through every query later - there is no admin-only page to guard yet, and the first one brings the gate and the first test of it. And **nothing about sharing**: sharing a workspace later attaches to the workspace rather than to the account, so it is purely additive and no table anticipates it.

**Interim note (2026-08-13): Cloudflare Access currently fronts both deployed environments, production included** ([deployment.md](deployment.md)'s "Secrets and access", §6). This does not reverse the decision below: Access is a perimeter around a deployment, not the application's identity model, and it exists precisely *because* the proof-of-identity half of this section is not built yet, so production would otherwise have no authentication at all while holding real mail and messages - a logon page you sign in at by clicking a name does not change that by one inch. It does mean the second rejection reason below is being lived with in the meantime, and it is a real cost, not a theoretical one: an expired Access session returns HTML to the client's background revalidation and SSE stream, where JSON or events were expected. **For reads this is now recovered from** — the client checks the ungated `/health`, concludes the deployment is fine and the sign-in is not, and sends itself through Access and back to the page it was on (§5.2's "The read model: persisted snapshot, revalidate, push"; the reasoning is in [deployment.md](deployment.md) under "The cost of gating production, stated plainly"). **The push stream recovers too, by a different route**: `EventSource` reports that it stopped but never why, so the stream is reopened on a backoff and the same `/health` check decides whether to say the sign-in has gone. Further mitigated with a long session duration. A second case is not mitigated by that at all: requests the browser makes with credentials omitted never offer the cookie in the first place, so a perfectly valid session does not help, and the recovery above cannot see it either — nothing was refused, so nothing asks why. The web app manifest is one of those, and behind Access it failed as a cross-origin CORS rejection rather than as anything that reads like an auth problem ([deployment.md](deployment.md)'s "Secrets and access", §6). **When password or OAuth login ships, Access on production gets reconsidered rather than left in place by inertia** - that, and not the name picker above, is the trigger.
...
- **Per branch** (the trigger is `push`, so draft PRs and branches without a PR are gated too): lint + typecheck + the connector-boundary import rules (§6.2); fast test tiers (L1, L2, F1, F2) in full, split as one job per tier so a misplaced test is visible (testing strategy §10); bundle-size gate; build. A per-branch **preview deployment** was part of this and is not any more: Cloudflare withholds version preview URLs from a Worker that implements a Durable Object, and gating a replacement turned out to cost per branch what previews got for free, so branches are now gated and deployed nowhere (deployment, "No branch environments").
...
- **On every pull request against `main`, and on `main` itself:** static analysis with CodeQL, over the application sources *and* over the workflow files. The second half is not decoration — this repository's own workflows run Claude against an OAuth token and check out pull request branches, which makes them the highest-value target it has. Alerts at high severity or worse turn the check red; the rest land in the Security tab without blocking. Which parts of this are repository settings rather than files, and how they were set, is in [deployment.md](deployment.md)'s bootstrap runbook.
...
**Decision: trunk-based development with one long-lived branch.** `main` is the trunk, every other branch is gated on push and deployed nowhere, merging deploys staging, and production is promoted deliberately. Full model and arguments in [deployment.md](deployment.md), which also holds the topology and the runbook.

**Two recorded corrections, both from building it:**

- **~~Previews share one D1 database; they are not isolated per branch.~~** Superseded: there are no previews. An earlier draft specified "an isolated preview D1 database" per preview; the shared one that replaced it was removed with the preview environment itself, once Cloudflare turned out to withhold version preview URLs from a Worker that implements a Durable Object and nothing cheap could gate a replacement (deployment, "No branch environments"). The isolation argument is kept there rather than repeated here.
- **Staging needed an environment, not a branch, and production needed a gate.** This section previously implied merge-to-`main`-deploys-production, and an intermediate draft gave staging its own long-lived `dev` branch. Both were wrong in the same place. The real requirement is that *something* runs every commit continuously before production sees it, because triggers attach to a Worker's *active deployment*: the per-branch preview versions of the time fired no §6.3 sync cadence, aged no token into a refresh and tripped no §9.2 dead-man's switch - and today nothing is deployed per branch at all, which only sharpens the point. Pointing staging at the trunk satisfies that with one branch; making production an explicit pinned promotion supplies the gate that merging used to provide. The two-branch draft bought the same soak at the price of a promotion merge and a `hotfix/*` path with a mandatory back-merge, which is machinery that exists only to reconcile long-lived branches that can diverge.
...
- **External uptime check** on a `/health` endpoint verifying that the register answers *and* that an account store can be opened and brought up to date: the only layer not running on the app's own code. Both halves, because once an account's data moved into its own store (see [account-storage-options.md](account-storage-options.md)) a check on D1 alone would report a deployment healthy while every request to it failed. The store it opens belongs to no account — the endpoint is deliberately unauthenticated, so it must never open a real one.
