...
**The app shell has one state, which is being inside a Workspace**, and everything account-level is a window over it. The header wears the Workspace's colour, holds its *Capture…* control and marks its tab, so an address reached without a Workspace had none of that: the chrome fell back to a default theme over a strip with nothing selected, which read as a second, worse header. The Workspaces and the Types were such addresses and are windows now, beside the Dashboards' — three lists, one shape ("Rename and delete a dashboard from a dashboard settings page", issue 90). The one case with no Workspace to be over is having none at all, and that is a screen of its own beside the logon page: it says so and offers the box to name your first.
