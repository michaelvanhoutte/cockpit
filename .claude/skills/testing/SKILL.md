...
**The one exception: a rule the real interface makes unreachable by construction.** The database constraints (architecture, "The database is the second lock") exist to catch writes the command handlers never validated - and the handlers both shape-validate with Zod and check that what a command names exists, so no request can drive an invalid value at them. A test entering through the interface could only prove the handlers work, which the request-validation tests already prove. `apps/api/tests/integration/db/constraints.test.ts` therefore writes to D1 directly, which is still entering the service's own real infrastructure rather than routing around it. Narrow on purpose: this applies when the interface *cannot* reach the behaviour, never when going through it would merely be inconvenient.

It cuts the other way too. Where a constraint *is* reachable from the interface, the caller-visible behaviour is the thing to test, and only a test through the interface can hold it: a capture naming a workspace that does not exist must come back 404, not the 500 a raw foreign-key error would produce. A direct-to-database test is structurally blind to that.
...
5. **A test that cannot fail is not a test.** See the whole table go red once before the implementation lands, and say so in the pull request. Where the implementation landed first, take the behaviour back out and watch the same table go red *then*, while the test is still in hand — not as a sweep at the end. A test never seen red passes for two indistinguishable reasons: the behaviour works, or it asks nothing. The obvious test for a stream error once passed identically with the fix, without it, and with its guard removed, because the runner could not produce the condition at all.
6. **A red check expires when the code it points at changes.** Red proves the test could fail against *that* implementation, not the one that ships. Reshape the code afterwards - for a review finding, for a bug the browser found - and take the check again, because a test can stop being able to fail without changing a character. Three did in one run: a migration case went vacuous once the handler stopped reading the column it folded; a preload case could never reach preload once a hover triggered it, which jsdom does not perform; an offline case re-read a copy too fresh to be re-read. All three had been seen red, all three passed at the end, all three asked nothing by then. A test that survives a rewrite of its subject untouched is the suspicious case, not the reassuring one.
7. **State a rule as what the person ends up seeing, not as what the function returns.** Correct logic wired to the wrong thing passes every test of the logic: a failure diagnosed right and then announced on a query no screen renders an error for, a guard cleared by the read that is not the one arming it. *The workspace shows the sign-in banner* cannot pass while the wiring is wrong; *diagnose returns signed-out* can.
