...
**D1 still holds the four tables an account's data used to live in.** Removing them is a *contract* step for a later release, per "Migrations and rollback" in [deployment.md](deployment.md): a deploy applies migrations before the new code goes live, so dropping them in the same release would leave the old code reading tables already gone — and re-promoting the previous commit, the first way back, would leave it that way. Undescribed is not untouched: three of them point at `tenants` under `ON DELETE RESTRICT`, so deleting an account takes its rows there too, or the account row is refused — `alsoWhereItUsedToLive` in `accounts/register.ts`, which asks `sqlite_master` first so it becomes a no-op the day they go.
...
**The admin pages are outside all three, and are the only thing that is** ("Add a user on the admin page, so a second person no longer needs SQL", issue 231): `/v1/admin/` is mostly the *register*, which is the environment's rather than an account's. None of what the three patterns buy applies to it — there is no snapshot to derive a page from, nothing to cache for offline, and nothing to replay, since a command is addressed to one account's store and adding a person is what creates one. So they are ordinary requests, and a write there is REST — `POST /v1/admin/<what>` to make one, `PATCH /v1/admin/<what>/{id}` to change one ("Rename a user, and make somebody an admin", issue 232), `DELETE /v1/admin/<what>/{id}` to take one away ("Delete a user, and the account they owned with them", issue 234) — rather than a command.

**Two of them reach past the register into an account's store**, and they are the only ones: `GET /v1/admin/users/{id}/account` counts what somebody's account holds so the question asked before deleting them can say what goes, and the `DELETE` destroys it. Neither is a command — a command is a change an account records and replays, and destroying an account is the one thing there is nothing left to replay into. They are also the only two reads of a store that do **not** bring the account up to date first, for the reason `holdsAsItStands` gives: bringing an account nobody has opened up to date is what creates the workspace it starts with, so counting that way would make the account hold what it was counting.
