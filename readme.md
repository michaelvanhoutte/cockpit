...
|---|---|---|
| [CI](.github/workflows/ci.yml) | push to `main`, and pull requests to `main` | Eleven jobs, so a failure names itself: `What changed` runs first, five wait on it, three more (`Scripts`, `Concepts`, `Stability`) run beside them, `Test Explorer` waits on `Test` and `Concepts`, and `Publish` waits on `Test Explorer` and `Stability`. `Typecheck`, `Lint`, `Test`, `E2E (F3)`, `Build` and `Scripts` are six of the eight contexts the branch-protection payload lists (the other two are the Claude reviews); `Concepts` checks the test-explorer registry, `Test Explorer` and `Stability` publish reports, and all three deliberately do not gate — `Publish` puts what the latter two produced on [GitHub Pages](https://michaelvanhoutte.github.io/cockpit/) from `main` only. `What changed` gates nothing itself — it classifies the diff (`scripts/lib/what-changed.mjs`, taken from the base commit so a branch cannot rule on its own), and the five jobs waiting on it skip their work on a diff confined to `docs/`, `.claude/` and root-level Markdown, still reporting as skipped ("Skip the mechanical checks on a pull request that touches nothing they cover", issue 345); `Test Explorer` then skips too, since it waits on `Test` and a skipped dependency is not a successful one. `Lint` runs [`eslint.config.mjs`](eslint.config.mjs) over the repository and then the tests that hold its `files` globs — the three rules being the React Hooks pair, no focused test reaching `main`, and the L1/F1 dependency table as restricted imports. `E2E (F3)` installs Chromium and runs the browser tier against the same isolated local stack you get locally, keeping failure traces as an artifact. `Scripts` runs the unit tests for `scripts/lib/`, because a silent change in the test stack's guards would let a run start against a database it did not create, and one in the review gate would let a non-review ship green. `pull_request` — which tests the merge result, what actually lands — is the only event that checks an open branch, so a branch with no open pull request is checked by nothing; [`/issue`](.claude/commands/issue.md)'s own process already runs typecheck, lint, the tests and a browser pass by hand throughout a unit of work, and opens the pull request only once that is already done. |
