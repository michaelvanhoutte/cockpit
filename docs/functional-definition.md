...
- **Admin pages** — where an admin manages the Cockpit rather than an Account: today, who can sign in at all, and adding somebody ("Add a user on the admin page, so a second person no longer needs SQL", issue 231). **Adding takes a name and the Google address they sign in with, and nothing else**: everyone arrives ordinary, and the Account they own is made with them rather than chosen. Two people may share a name — the address is what a Cockpit holds to one person — and somebody added is in the register before they ever sign in, which is what makes their first sign-in possible at all. **The one part of the app that crosses Accounts**, since who exists is the environment's question and not any one Account's; and the one management screen with an address, for the same reason — there is no Workspace behind it to keep.
