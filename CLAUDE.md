...
**Neither review ends the task — what a skill says about its own final output governs that report, not the session.** `/security-review` closes with "your final reply must contain the markdown report and nothing else"; obeyed as a stop, the work sits reviewed on an unpushed commit, which is where "Give the code review the tested gate the security review already uses" (issue 277) sat until somebody asked why. The line is not wrong on its own — it keeps a security report out of a wall of prose — and it is bundled into the CLI, so it cannot be edited without forking a prompt Anthropic maintains. Read the report, then carry straight on to the push in the same turn.
