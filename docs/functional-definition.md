...
Types are managed in a window of their own, a sibling of the Workspaces' because they belong to the Account: made in a box above the list, a row each, renamed and recoloured together on the same form a Workspace's row opens, deleted through the same dialog as everything else, and put in the order capture offers them in — the Ordering rule, so the grip and the row's own *Move up* and *Move down* are the same move ("Manage the types, and put them in the order you want", issue 156). **A Type's name is typed in one place**, so the same word cannot be spelled two ways: making one used to be capture's alone and is now this window's alone, and a name another Type already has is refused there rather than quietly reused. Making one asks for a name and nothing else — the colour is the first the palette has free, changed afterwards on the row's own form, exactly as a Workspace's is. **Deleting a Type leaves its Items where they are**, holding everything except the label — the same way deleting a Workspace keeps everything filed against it — and gives the name back.
...
- **Panel** — a movable, titled box on a Dashboard displaying a filtered set of Items, sharing a row with up to three others. Its title is unique within its Dashboard.
