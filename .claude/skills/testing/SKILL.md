...
6. **State a rule as what the person ends up seeing, not as what the function returns.** Correct logic wired to the wrong thing passes every test of the logic: a failure diagnosed right and then announced on a query no screen renders an error for, a guard cleared by the read that is not the one arming it. *The workspace shows the sign-in banner* cannot pass while the wiring is wrong; *diagnose returns signed-out* can.
