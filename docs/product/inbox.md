...
**Capture is also a screen of its own**, reached from the header and belonging to no Workspace: a note of several lines, the Types as chips, and a row of Workspaces that starts on *Any workspace*. Left there, the note waits in every Workspace's Inbox until somebody says where it belongs; the Workspace it was captured *from* is recorded either way. What it has just captured is listed under the box for as long as the screen is open. The Inbox's own row stays the narrow way in — it has a column to fit in, and asks the same capture in one line.
