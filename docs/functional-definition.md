...
**The band under the Workspace tabs names the screen you are on, at every address.** Inside a Workspace it holds the Inbox's heading and the Dashboard tabs; where you are in none — the two settings pages — it holds those two as tabs, *Manage workspaces* and *Manage types*. So a settings page has no heading of its own, and the chrome does not change height between one address and another. The settings pages are read rather than worked in, so their content is a column of prose width inset from the window rather than a sheet running to its edges.
