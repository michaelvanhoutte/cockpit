...
/**
 * "Scroll the dashboard or a panel while dragging near its edge" (issue 524).
 * A panel's header drag is a pointer gesture the browser never scrolls for, so
 * this is the only place anything proves the dashboard follows it - and that
 * the arrangement drawn under the hand follows the scroll while the pointer is
 * still. When and which box is apps/web/tests/unit/dragScroll.test.ts.
 *
 * Desktop only, like the other panel drags.
 */
test.describe('Panels', () => {
  test.describe('a drag reaches a place that was not on screen when it started', () => {
    test.skip(({ isMobile }) => !!isMobile, 'dragging a panel is a pointer gesture');

    test('scrolls the dashboard while a panel is held at its bottom edge, until the gap under the last row is on screen, and drops there', async ({
      page,
      isMobile,
    }) => {
      await ownDashboard(page, isMobile);
      await page.setViewportSize({ width: 1280, height: 560 });
      const names = ['A', 'B', 'C', 'D', 'E', 'F'].map((n) => uniqueTitle(n));
      for (const name of names) await addPanel(page, name, isMobile);
      await expect.poll(() => rowsOnScreen(page)).toEqual([
        [names[0], names[1]],
        [names[2], names[3]],
        [names[4], names[5]],
      ]);
      ...
      if (!over) throw new Error('the dashboard is not on screen');
      await page.mouse.move(
        ...(await centreOf(page.getByRole('region', { name: names[0]! }).locator('header'))),
      );
      await page.mouse.down();
      const x = over.x + over.width / 2;
      await page.mouse.move(x, over.y + over.height - 6, { steps: 8 });
      // Held still but for a pixel. The scrolling is a frame loop, and the
      // pointer firing nothing is the case the drawing must follow anyway.
      for (let held = 0; held < 200 && !(await gapInView()); held += 1) {
        await page.mouse.move(x, over.y + over.height - 6 + (held % 2));
        await page.waitForTimeout(40);
      }
      expect(await gapInView(), 'the gap never scrolled into view').toBe(true);

      await page.mouse.move(...(await centreOf(gap)), { steps: 4 });
      await page.mouse.up();
      expect((await saved).status()).toBe(200);
      await expect
        .poll(async () => (await rowsOnScreen(page)).at(-1))
        .toEqual([names[0]]);
    });
  });
});
