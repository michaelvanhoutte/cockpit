...
4. If the issue (or the unit being built) carries a `## Model` section — scoping's sizing step adds one only where the session default is wrong for the unit — do steps 4–7 inside an `Agent` tool call using that model override rather than in this session; the calling session stays whatever it started as. Otherwise continue here. **Build**, following the conventions already in the code it touches — `packages/shared` is the contract.
...
7. **Review yourself before pushing, against the commit that's actually about to be pushed**: `/code-review xhigh` and `/security-review` on the diff, per **Review findings**. Re-run it if the diff changes afterward — a review of a commit that's since been replaced proves nothing — and don't mark ready until its findings are fixed or declined.
