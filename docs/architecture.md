...
**D1 still holds the four tables an account's data used to live in.** Removing them is a *contract* step for a later release, per "Migrations and rollback" in [deployment.md](deployment.md): a deploy applies migrations before the new code goes live, so dropping them in the same release would leave the old code reading tables already gone — and re-promoting the previous commit, the first way back, would leave it that way. **Deleting a user clears that account's rows from them** in the same write that removes the user ("Delete a user, and the account they owned with them", issue 234): three of them hold `tenants` with a restricting foreign key, so the register row could not go otherwise — and the release that drops them takes those statements with it, which the deletion tests fail on until it does.
...
- **Two commands carry no client and no route.** `propose_item_texts` and `propose_item_panel` are sent by the enrichment job over the account's own store, never by a browser, so neither is mounted in `apps/api/src/http/app.ts`. They are in `commandSchemas` because that is what types the store's one write path, not because they are reachable from a client — `SelfSentCommandName`/`ClientCommandName` keep them apart so a command a browser cannot send fails the client's typecheck rather than getting published as a dead endpoint. There were three: `write_routing_summary` went with the nightly job that was its only sender ("Drop the nightly filing summary, keep the sentence you wrote", issue 392).
...
| `set_routing_summary_correction` | The empty string clears it, unlike `set_description`'s `null` for the same idea — this field has no third state to spend `null` on (`domain/routing-summary.ts`, `packages/shared`). Upserts `workspace_routing_summary`, writing only its own two columns — now the only two anything writes, `summary`/`summary_generated_at` having been left behind, unread, by the job that wrote them ("Drop the nightly filing summary, keep the sentence you wrote", issue 392). |
...
- **`panelKindSchema`** — items, text ("Put a panel of text on a dashboard, and write in it", issue 250) or filter ("Add a Filter panel that shows every filed item due in a window", issue 463) — is decided when a panel is made and never after; turning an items panel into text (or back) is a question about what happens to what is already in it, and nobody has needed it answered. `'items'` is the default both ways: on read, so a panel stored before kinds existed reads as one; on write, so a client that has never heard of kinds adds the panel it always added. **`panelTakesItems`**, **`panelHoldsText`** and **`panelGathers`** are functions rather than the comparison written at each call site — the server refusing a filing, the server offering panels to a routing proposal, and the client's own picker — so they cannot come to disagree about what a panel will take; `refuseAPanelNothingIsFiledOn` in `apps/api/src/accounts/command-service.ts` enforces the same shape of rule. Adding the filter moved what `panelTakesItems` means out from under a caller that was asking it — which is what a named question survives and a bare comparison spelled out at each call site does not.
- **`STORED_PANEL_KINDS` is the two the `kind` column may hold, and a Filter is not one of them.** It is stored as a panel of items with `filter_conditions` set, and `panelAsRead` (`apps/api/src/accounts/repo.ts`) is the one place that becomes a wire kind of `filter`. Widening the column's CHECK would mean rebuilding `panels`, which filings, placements and an Item's proposed Panel all point at under RESTRICT — the wall "Take the width and the name off a layout, now that its size carries them" (issue 264) hit on `layouts` — so the change is one nullable `ADD COLUMN`. It is also what makes the release rollable back: an older release reads a Filter as an empty panel of items rather than as a kind it has never heard of, and will accept a filing onto it, which is why the client counts a filing onto a Filter as no filing at all (`filingsThatFile`, `apps/web/src/filing.ts`).
...
- **`registeredUserSchema`** is deliberately more than `userSchema` tells anyone about anyone else, since it is what the admin pages alone see. `lastSignedInAt` rather than the Google identity itself — what an admin needs is when a person last proved who they are, which is recorded at every fresh sign-in (`auth/register.ts`'s `startVisit`) and left alone by a session merely renewing itself, without the identity Google keys them by ever being a thing to publish; `null` reads as "not yet" ("Show when each person last signed in, on the admin page", issue 342). `email` is nullable because the register's own column is: it arrived after the rows did ("Record the Google account each user signs in with", issue 195), so a row without one is a person nobody can sign in as, worth showing rather than hiding. `disabled` is a boolean rather than a timestamp, since when it happened is not a question this page asks and there is no audit trail to put it in ("Take somebody's access away without taking their work", issue 233).
...
- **`accountHoldingsSchema` is a read of its own, asked when the question about deleting somebody opens** ("Delete a user, and the account they owned with them", issue 234), rather than a count on every row of the list: counting for the list would wake every account's store to draw one page. It is counted as the store stands, so looking never creates or migrates anybody's account. `empty` travels beside the count because a deleted workspace keeps what was in it: no live workspaces is not nothing held, and "there is nothing in their account" is said only of the second. `userDeletedSchema` says only that it happened, since there is no row left to answer with.
...
- **`duplicates` is a flat list of pairs, and never the vectors behind them** ("Flag a captured note that says what another one already said", issue 407) — what an Item means is a thousand numbers, kept in a table of its own inside the account's store, and the screen needs no more than "this one, and that one". The store leaves out every pair a person could no longer act on or that belongs to another workspace; whether a pair is *drawn* is a view over the same snapshot, since a filed Item is nothing's duplicate yet.
...
- **A second provider, behind a second interface of the same shape** (`embeddings/`): Workers AI reading what a note *means*, as a vector, so that one saying what another one already said can be flagged ("Flag a captured note that says what another one already said", issue 407). Its own service rather than a method on the one above because Anthropic has no embedding model, and a binding rather than a key — `env.AI`, separately configured and separately absent, which is why `/health` answers `embeddings` beside `ai` and why an environment can have either, both or neither. Its results are the one enrichment not written onto the Item: a reading is a thousand numbers, so it and the pairs drawn from it live in tables of their own and reach the wire as pairs of ids.
...
- **No pull request merges without the judgement half's pass.** CodeQL is the mechanical half, reading the sources *and* the workflow files — the second half because this repository's workflows run Claude against an OAuth token and check out pull request branches, making them its highest-value target — but it no longer runs on a pull request at all (§9.1): it analyses `main` on merge and on a weekly schedule instead ("Decide whether CodeQL earns its run on every pull request push, or moves to main and a schedule", issue 379), so a high-severity finding there turns the check red within a week of merging rather than before. The judgement half is a Claude pass over every diff, scoped by a checked-in instructions file and required to end with an explicit verdict, so a run that never reached one is red rather than silent — it does not model this document's own rules either, but it is the only one of the two that sees a pull request before it merges, so the instructions file tells it not to assume CodeQL already covers this diff's injection, taint and workflow misuse.
...
- **On merge to `main`, and on a weekly schedule:** CodeQL over the application sources *and* the workflow files (§8). High-severity alerts turn the check red; the rest land in the Security tab. No pull request waits on it — "Decide whether CodeQL earns its run on every pull request push, or moves to main and a schedule" (issue 379) moved it off the pull-request gate, so an alert now surfaces within a week of merging rather than before. Which parts are repository settings rather than files is in the bootstrap runbook in [deployment.md](deployment.md).
- **On a diff that touches only `docs/`, `.claude/` or a root-level `*.md`:** the five gated checks — lint, typecheck, the fast tiers, the browser tier, build — skip their work and report as skipped, which a required check accepts where a workflow-level path filter would report nothing at all ("Skip the mechanical checks on a pull request that touches nothing they cover", issue 345). Lint, typecheck and build are steps of `ci.yml`'s `Checks` job rather than jobs of their own ("Run the six small CI jobs as one, so a push stops paying six runner start-ups", issue 378); the fast tiers and the browser tier stay separate jobs. The classification is `scripts/lib/what-changed.mjs`, an allowlist, so an unrecognised path runs the lot, and it is taken from the base commit rather than from the branch, so a pull request cannot rule on its own diff. `Checks`'s own Scripts and Concepts steps are not gated, deliberately: the writing-rules test reads exactly the prose the others skip on. `Test Explorer` carries no gate of its own either, but skips anyway — it depends on `Test`, and a skipped dependency is not a successful one. The two remaining report jobs gate nothing regardless, and CodeQL no longer runs on a pull request at all.
