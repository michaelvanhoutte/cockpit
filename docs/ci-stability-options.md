...
- **Measuring what CI can see.** The known F3 instability is a Node crash on Windows ("Node crashes in its own socket code, taking the F3 stack down mid-run on Windows", issue 207), and CI runs Ubuntu. A green stability page is evidence about the runner, not about the suite on a developer's machine.
