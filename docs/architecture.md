...
- **App login per §8.1**; no passwords stored, ever. The session, cookie and request gate are built; the proof of identity in front of them is a logon page you sign in at by choosing a name — an identity selector, not an authentication control — so Cloudflare Access is what actually authenticates a deployed environment until that changes.
...
- **Cloudflare Access stays in front of every deployed environment, unreconsidered.** The trigger for reconsidering it is password or OAuth login shipping — not this.
...
**Interim note (2026-08-13): Cloudflare Access fronts both deployed environments, production included** (see "Secrets and access" in [deployment.md](deployment.md)). This does not reverse the decision: Access is a perimeter around a deployment, not the application's identity model, and it exists precisely *because* the proof-of-identity half is not built, so production would otherwise hold real mail with no authentication at all. The cost is real: an expired Access session returns HTML to background revalidation and to the SSE stream, where JSON or events were expected. **Reads recover from this** — the client checks the ungated `/health`, concludes the deployment is fine and the sign-in is not, and sends itself through Access and back to the page it was on (§5.2). **The push stream recovers by a different route**, since `EventSource` reports that it stopped but never why: the stream reopens on a backoff and the same `/health` check decides whether to say the sign-in has gone. A second case is not mitigated at all: requests the browser makes with credentials omitted never offer the cookie, so a valid session does not help and the recovery cannot see it — nothing was refused, so nothing asks why. The web app manifest is one of those, and behind Access it failed as a cross-origin CORS rejection rather than as anything reading like an auth problem.
