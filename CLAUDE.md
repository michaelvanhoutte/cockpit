...
**A red check on your own pull request is not one of those**, whatever area it comes from: it blocks the work rather than sitting beside it, so settle it in this pull request — fix it, or quarantine or delete the test per the `testing` skill's *Flakiness* — once `gh pr list` says nobody is already on it. "Delete a user, and the account they owned with them" (pull request 362) chipped the flaky walks blocking it and asked twice about the rest, at three CI rounds.
