...
- **Deadline colors.** *(Design decided, not built: no component reads `dueDate` for it.)* Independently of Focus horizons, an Item can carry a hard due date, and cards would be color-coded by proximity: neutral while there is time, orange once the deadline is reached (due today, or within a set threshold), red once passed, in every Panel the item appears in.
