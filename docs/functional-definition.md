...
**Workspace** — the top-level context and the privacy boundary. Examples: *Work*, *Personal*, *Home*, *Customer 1*, *Customer 2*. A Workspace controls **which source accounts are connected and visible**, so when you are in the *Work* workspace you do not see private email, and vice versa. This is the "I don't want to see private emails while processing work stuff" requirement, solved by scoping rather than by filtering after the fact. (Original notes called this level "dashboard" and were unsure; "Workspace" is the recommended name.) Each Workspace also has its own **color identity**, and it is four colors rather than one: a saturated tint on the tab dot and the tab you are on, and three surfaces that step from deepest at the top of the screen to lightest at the bottom — the header bar across the top, the strip the Dashboard tabs sit on one step lighter than it, and the page ground behind the panels. Switching Workspace repaints all four, so it is always obvious at a glance which Workspace you are currently in. Nothing else moves — cards, rows, controls and text keep the fixed neutral and accent palette — which is what lets the colors be chosen without anything becoming unreadable. They are picked as a set from a fixed palette on the workspace settings page (open decision #13).

**The two tab strips are joined to what they own** ("Modernise the app shell: a fourth workspace colour, connected tabs, and Inbox rows you can read at a glance", issue 125). The Workspace tab you are on is filled with the strip's color and runs into it with no edge between them; the Dashboard tab you are on is filled with the ground and runs into the page the same way. So the container hierarchy is legible as depth rather than as two rows of pills on one flat fill, and which Dashboard of which Workspace you are looking at is one glance instead of two readings. That is what the fourth color is for: three surfaces are what two joined strips need, and the tint alone was never going to carry it.

Workspaces are made, renamed and deleted from the workspace settings page, where each one is a row carrying a menu of what can be done to it. A name is required, is stored with the surrounding blanks removed, is one line of at most 60 characters, and no two live Workspaces share one whatever the capitalization. **Deleting a Workspace keeps everything that was in it.** It leaves the tabs, the settings list and everything its contents could be read from, and its Items stay filed against it: the router learns from the whole history of where things were actually filed (routing that learns from past decisions, "What the model reads: the whole history, no retrieval"), so erasing a Workspace would erase that signal along with it. The name becomes available again. The last Workspace can be deleted, and the app then opens on the page that makes one.
...
13. **Workspace colors.** Auto-assigned from a palette or user-picked per Workspace? *Recommendation: user-picked with sensible defaults; ensure the palette stays accessible/legible in both light and dark mode.* **Answered: both, in that order.** A Workspace made in the app is given the first color no other Workspace is using, so it never exists without an identity and nobody is asked for one to create it; the workspace settings page then offers a row of swatches to change it. What is picked is a **theme of four colors** — the saturated tint on the tab dot and the tab you are on, the header bar across the top, the strip the Dashboard tabs sit on one step lighter than it, and the page ground behind the panels — and all four are stored on the Workspace, so offering a free color wheel later is a second writer of the same four fields rather than a migration. The fourth arrived with the joined tab strips ("Container hierarchy") and is stored rather than mixed from the two it sits between, for the same reason the other three are: an entry stays tunable by hand, and the palette stays a picker rather than a storage format. Legibility is kept by the palette being a fixed set designed together rather than four independent choices, and by nothing else recoloring: cards, rows, controls and text keep the fixed neutral and accent palette whatever is chosen. Dark mode is not built yet and is not decided here; when it lands, each theme gains its dark set beside its light one — four colors now rather than three, which is the cost of the fourth and is worth naming.
