...
**That transient is patched out rather than tolerated.** It was not rare: every walk opens the live-updates stream and walks away from it, and three of the four CI browser failures of 4 and 5 September 2026 were Wrangler quitting on one — in a different walk each time, which is why they read as several flaky specs rather than one dead server. A pinned `pnpm` patch adds that reason to the two Wrangler already treats as non-fatal (`pnpm-workspace.yaml`, which carries the reasoning and what a version bump costs). Two consequences worth knowing: the same record is now ordinary noise in Wrangler's log, so `fatalReason` reads past it (`scripts/lib/stack.mjs`); and the noise the same event makes in the *Worker's* log is a different thing again, owned by ["Close the live-updates stream quietly when a browser walks away" (issue 128)](https://github.com/michaelvanhoutte/cockpit/issues/128).
