...
- Where tooling allows, enforce mechanically rather than by review: CI jobs split per level, the §7 budget checked in CI, and the parts of §3 a rule can hold. Prefer making violations impossible over catching them in review.
- **What is enforced today is `eslint.config.mjs`**, run by CI's `Lint` job: a filesystem, network or database import under a `tests/unit/` folder fails, as does a focused test in any runner's test file. Not the API client, which this section used to name — what the frontend unit tests import from it is the refusal error class and the module they then mock, neither of which reaches the network, so the ban would fail tests that are at the right level.
