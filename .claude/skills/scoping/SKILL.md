...
- **What is actually in each environment?** Read the deploy workflows rather than assuming, because they differ on purpose: staging deploys every commit on `main` and is deliberately never re-seeded, and production is promoted by hand.
