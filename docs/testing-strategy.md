...
**A walk waits for the change, not for the picture of it.** Twice an intermittent CI failure has been a walk acting on something the browser had drawn but had not yet sent or read back: a workspace switched but not yet loaded, and a panel resize drawn on every pointer move and saved only when the hand stops — reloading over that request cancelled it, and the panel came back the width it started at. That gesture has since gone, and what replaced it has the same shape: a dashboard draws the arrangement a drag produces before it sends it, so the panels look identical either side of the save. Where something on screen tells "drawn" from "kept" apart, wait on that; where nothing does, wait on the server's answer to the command (`answerTo`, `tests/e2e/panels.test.ts`).
