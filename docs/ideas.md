...
- **A Panel that shows text.** Two variants, neither decided: a Panel whose contents are text, and a Panel with text sitting between the Items. Not the plain-English description box above, which is configuration — what the Panel should render — rather than something read on the Dashboard. The cost is the renderer: `RichDescription` is loaded lazily so it never reaches the cold-open path, and a Panel rendering Markdown on every Dashboard load has to answer the performance budget. *(Raised while scoping new-user onboarding, which needs prose in a Panel and takes it from the Panel's empty state instead, because its lines have to go as each container is named and stored text cannot.)*
