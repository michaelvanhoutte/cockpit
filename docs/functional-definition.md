...
**Panels** behave like the tiles on Azumuta's Analytics dashboards: each sits on a row, **drag to move**, an editable **title**, and a **menu of its own** (configure, rename, remove) opened by the same control as every other menu in the app ("One control opens every menu"). **Adding a Panel is a control on the Dashboard's own bar** ("Layouts: the arrangement you pick by name"), beside the one naming the arrangement the new Panel joins. Rows reflow on smaller screens so the same Dashboard works on phone and desktop.
...
**Which Layout a Dashboard is drawn with is a control at the right of its own bar**, beside the tab whose arrangement it names, along with the way to add a Panel. Both are drawn only where a Dashboard is open, since the bar is also the Inbox's. The control names the Layout in use; its menu lists **Automatic** and each Layout by name with the width it was made at, and offers *New layout from this one*, *Rename* and *Delete*. **Automatic is an entry rather than the absence of one**: it draws the Layout whose recorded width is closest to the screen in front of you, a tie going to the narrower so two devices of the same size agree, and the control says so and says which one it landed on. It is what a Dashboard nothing has been picked on gets. The choice is remembered in the browser rather than stored — the whole point of Layouts is that the phone and the desktop want different ones. A Layout that is deleted, or a hand-picked one that another device deletes, simply falls back to the closest remaining.
