/**
 * Whether an error is Cloudflare saying the free tier's daily Durable Objects
 * allowance is spent, rather than anything wrong with the data.
 *
 ...
 * API carries: "Exceeded allowed rows read in Durable Objects free tier", its
 * "rows written" twin, and the other daily limits worded the same way.
 */
export function allowanceSpent(error: unknown): boolean {
  const message = error instanceof Error ? error.message : String(error);
  return /exceeded allowed [\w ]+ in durable objects free tier/i.test(message);
}
...
export const ALLOWANCE_SPENT_MESSAGE =
  "the daily Durable Objects allowance on Cloudflare's free tier is spent; it clears at 00:00 UTC, or on Workers Paid";
