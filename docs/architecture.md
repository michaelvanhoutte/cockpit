...
All writes go through small, named, idempotent commands: `capture_item`, `set_done`, `set_dismissed`, `associate`, `set_next_action`. Each carries a client-generated command ID, the client timestamp, and a minimal Zod-validated payload from `packages/shared`.
