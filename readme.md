...
**Why the gate asks about the head, and not about the pull request.** Having posted *at all* answers for the first round and for no round after it — round one's comments satisfy that test permanently, so from round two on the check could not tell "reviewed, found nothing" from "declined without looking". That is "The review check goes green when the reviewer declined to look at the new commits" (issue 75), and it ran four rounds deep on "Make managing the account a window over the workspace, not a screen instead of it" (pull request 193) before anyone opened the log. So the gate counts only what the reviewer said about the commit *this run* was handed: an inline finding by the commit it was made on — never the one GitHub later moved it to — and a summary comment by its timestamp, that being all an issue comment carries. Where GitHub cannot be asked when the head arrived, the check falls back to the whole pull request and says in a warning that it proved less. The reviewer is told the same rule in the appended system prompt, since a decline the gate merely refuses would go red every second round instead of being reviewed.
