...
**`claude-review` passes once it posts about the head, whatever it posted.** `Security review` and the bare `CodeQL` check are what actually gate on severity; this one doesn't. "Add a harness-cost-review skill: file candidates to tighten or drop a required check" (pull request 332) ran the full draft-push-ready cycle four times after the check had already gone green on the first pass, once per comment, because every posted finding was treated as blocking when only the first round's actual `failure` — a permission-denial flake, not a finding — ever was. Batch what you already have before pushing again; a fix rushed out to clear a check that was never red is exactly how that same pull request introduced a fresh mistake fixing an old one.
