...
**Every Item has a Type** — *Task*, *Note*, or whatever else you name — which is what kind of thing it is, as against where it stands. The set is open and account-wide. Capture offers the types you already have, the three used last first, opens on the one used last, and *No type* is one of the answers; it does not make one ("Make a type where types are managed, not while capturing", issue 203).
