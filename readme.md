...
**The gate writes the summary comment, not the reviewer.** The prompt used to ask the reviewer to post one when it found nothing, and on the very first run that reached a verdict it posted nothing at all — a green check whose only evidence was inside the job log, which is the state this gate exists to refuse. Asking harder would have repeated the sibling workflow's mistake of inferring a review from whether Claude spoke, which is what "The review check goes green when the reviewer declined to look at the new commits" (issue 75) is open about. The gate already knows the verdict, so it says so itself, editing its own previous note rather than adding one per push. Posting is never fatal: a comment that cannot be left is a warning, because failing a clean review over it would be the gate doing the thing it refuses in the reviewer.
...
One consequence worth knowing, and it is the action's behaviour rather than either workflow's: it refuses to run when the workflow file differs from the copy on `main`, so **a pull request that edits `claude-code-review.yml` is not reviewed by it, and one that edits `claude-security-review.yml` is not security-reviewed by it**. Both gates make that red rather than green, on purpose. The practical cost is that a fix to either workflow cannot be watched working on the pull request that makes it — the first run that can prove anything is the one after it merges.
