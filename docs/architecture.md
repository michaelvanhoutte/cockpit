...
4. **More than one user, SaaS-ready tomorrow.** Per the functional definition's "Decisions already made": never hard-code how many accounts there are. Every row is tenant-scoped, and since "Sign in by picking a name, each user in their own account" (issue 86) that scoping is exercised rather than assumed - each user signs in, and the account every request resolves is the account of whoever signed in. Auth is real OAuth *eventually*, and today is not that day (§8.1); secrets are stored encrypted, and workspace scoping is enforced server-side.
