 ...
 *
 * `panelId` is the panel its first dashboard arrives with. Generated by the
 * client for the reason the workspace's own id is - the same id has to come
 * back on a replay, and nothing on the server can derive a uuid. There is no
 * `dashboardId` beside it because a workspace's first dashboard takes an id
 * derived from the workspace's (`firstDashboardId`), which a panel cannot: five
 * commands take a `panelId` and every one of them wants a uuid, so a derived
 * one would leave the panel unable to be renamed, deleted or filed into.
 */
