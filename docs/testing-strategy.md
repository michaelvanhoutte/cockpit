...
- **A capability gets one browser walk; a second needs different setup, not a different assertion.** Before adding a `test()`, extend the walk in that `describe` which already reaches the state you need; start a new one only where getting there costs as many steps as starting over. Before adding any walk, read the F1 rules for that area — where the proposition is already stated there, the walk is duplication. The bullet above did not hold the tier because it asks whether a walk needs a browser and the answer is usually yes: F3 reached 110 walks in 12.3 minutes, `item-editing` making 115 setup calls for its 33 of them ("Bring the browser tier back inside a budget", issue 509).
- **Each product area has a ceiling on its walks, and passing it fails the run.** `browserWalks` in `tools/test-explorer/concepts.json` is the number, beside the area it belongs to; `pnpm test:e2e:ceilings` holds it in the `E2E (F3)` job, ahead of the browser download. Counted from `playwright test --list` rather than from the source, because a walk whose cases come from a table is one `test()` call and several walks. Per area rather than one number for the tier, so areas budget separately and one cannot spend another's room. Raising a ceiling is an edit in the same pull request as the walk that needs it, which is the whole point of holding one.
...
Level separation and one-command-per-level runnability are what is mandatory. Per the levels-are-roles rule, levels with no reason to exist yet stay absent — `system/` and `service/` are unbuilt today. `packages/connectors/teams/tests/` holds both of its folders as of "Save a Teams message to Cockpit" (issue 486): its `contract/` asks Microsoft whether it still publishes the signing keys, the issuer and the algorithm the connector checks a saved message against, and needs no credential, those keys being published to anyone. `apps/api/tests/contract/` is built, and is deliberately excluded from the config every other tier runs under: `pnpm test` is `vitest run` over everything under `tests/`, and a tier that spends real money on every run must not be reachable by an ordinary test command.
...
**A walk waits for the change, not for the picture of it.** Three times an intermittent CI failure has been a walk acting on something the browser had drawn but had not yet sent or read back: a panel resize drawn on every pointer move and saved only when the hand stops, where reloading over that request cancelled it and the panel came back the width it started at; and, twice, a workspace switched but not yet loaded, where the one being left stays on screen and usable, so what is captured next goes to it. That resize gesture has since gone, and what replaced it has the same shape: a dashboard draws the arrangement a drag produces before it sends it, so the panels look identical either side of the save. Where something on screen tells "drawn" from "kept" apart, wait on that; where nothing does, wait on the server's answer to the command (`answerTo`, `tests/e2e/panels.test.ts`). A switch has such a sign — the address gets deeper only once the workspace has been read — so every walk changes workspace through `switchTo` (`tests/e2e/support/app.ts`), and one that presses the tab itself fails a check over the walks' own source (`scripts/lib/e2e-conventions.mjs`). That rule was a comment the first time, which is why it was fixed once per walk rather than once.
...
**The Glossary in `docs/product/glossary.md` is the binding vocabulary**, and the architecture document is the mechanism whose words must not appear in anything the runner prints. This is checkable rather than a matter of taste: a word in `docs/architecture.md` but not in the product docs is an implementation word. "Command", "envelope", "tombstone", "idempotency" and "last-write-wins" are all real, correct, and belong inside the test body. Where the product does something the Glossary has no word for, the Glossary gains the word in the same change — a private synonym invented in a test file is how the two vocabularies drift apart.
...
- Where tooling allows, enforce mechanically rather than by review: network and filesystem access disabled in the unit runner, CI jobs split per level, the §7 budget checked in CI, and the parts of "Test level definitions and dependency restrictions" (§2) a rule can hold. Prefer making violations impossible over catching them in review.
- **What is enforced today is `eslint.config.mjs`**, run by CI's `Lint` job: a filesystem, network or database *import* under a `tests/unit/` folder fails, as does a focused test in any runner's test file. Not the API client, which this bullet used to name — what the frontend unit tests import from it is the refusal error class and the module they then mock, neither of which reaches the network, so the ban would fail tests that are at the right level. An import ban is the weaker half of the runner block above, which would also catch a global `fetch`.
- `CLAUDE.md` must require reading this document before writing or modifying tests, and must document the one-command way to start the application, because "the application has actually been started", in "Definition of done for agents", is only enforceable if starting the app is trivial.
