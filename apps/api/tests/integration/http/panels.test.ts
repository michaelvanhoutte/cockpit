...
describe('Panels', () => {
  ...
  describe('the layouts a workspace shows are its own', () => {
    it('leaves out another workspace’s, and a deleted dashboard’s', async () => {
      const mine = await aDashboard();
      const falcon = nextId();
      await addPanel(mine, 'Project Falcon', { panelId: falcon });
      await saveLayout(mine, nextId(), 1280, [{ panelId: falcon, columns: 4, rows: 3 }]);

      const doomed = await aDashboard();
      const passing = nextId();
      await addPanel(doomed, 'Passing thought', { panelId: passing });
      await saveLayout(doomed, nextId(), 1280, [{ panelId: passing, columns: 6, rows: 2 }]);
      expect(
        (await send('delete_dashboard', { workspaceId: WORKSPACE_ID, dashboardId: doomed })).status,
      ).toBe(200);

      // Every one of these is checked, because the assertion below is that the
      // other workspace's layout is absent - and a setup that quietly failed
      // would make it absent for the wrong reason and pass.
      const elsewhere = nextId();
      const stranger = nextId();
      const theirs = nextId();
      for (const made of [
        await send('create_workspace', { workspaceId: elsewhere, name: `Personal ${seq}` }),
        await send('add_dashboard', {
          workspaceId: elsewhere,
          dashboardId: theirs,
          name: 'Someone else’s',
        }),
        await addPanel(theirs, 'Not yours', { panelId: stranger, workspaceId: elsewhere }),
        await send('save_layout', {
          workspaceId: elsewhere,
          dashboardId: theirs,
          layoutId: nextId(),
          screenWidth: 1280,
          placements: [{ panelId: stranger, columns: 12, rows: 4 }],
        }),
      ]) {
        expect(made.status).toBe(200);
      }

      const shown = (await snapshot()).layouts;

      expect(shown.map((layout) => layout.dashboardId)).toEqual([mine]);
      expect(shown[0]?.placements).toEqual([{ panelId: falcon, columns: 4, rows: 3 }]);
    });
  });
});
