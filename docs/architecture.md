...
- **The two prefixes say which gate they are behind**, since "Give the operator's routes the operator's name, and free /v1/admin/ for the admin section" (issue 229): `/v1/operator/` is the secret's, `/v1/admin/` is the sign-in's plus the `admin` role. The operator's routes held `/v1/admin/` first, which named neither their caller nor their gate and stood on the address the admin pages need. The old subtrees answer `410` naming the new one, because whoever still asks there is a command line from a checkout that has not been updated, and "sign in to continue" is advice it cannot take.
