...
- ~~**Fixed-size panels are probably not good enough.** Panels may have to resize.~~ *(Taken up by "Drag a row taller and a panel wider, on the dashboard itself" (issue 267): the line under a row sets how tall it is and the line between two Panels sets how much of the row each takes.)* **The related open question stands**: how do Panel positions change when the Inbox opens or closes?
